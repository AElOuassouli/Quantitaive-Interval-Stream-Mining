...
from algorithms.TEDDY import Teddy


def CTDMinerTeddy(streams : [Stream], tmin, tmax, duration):
    final_results = []
            ...
            if process :
                for c in streams :
                    if c.m_label not in p.m_labels  :
                        results, confidences, transformations = Teddy(p.m_intersection, c, tmin, tmax, duration) 
